...
不同的MOF拥有不同的吸附性能，比如[第一章的1.2 典型结构示意与晶胞](./ch1-mof-basics#target-struct-zh)中示例的MOF结构。
...
:::{note}
上述图表展示了不同 MOF 材料的吸附特性。Langmuir 模型是简化模型，实际 MOF 的吸附行为可能更复杂，包括多位点吸附、微孔填充等机制。
:::
...
:::{tip}
上述可视化展示了气体分子在 MOF 孔道中的扩散过程。在实际应用中，可以使用更复杂的模型（如分子动力学模拟）来研究不同 MOF 结构的扩散特性。
:::
...
GCMC 是模拟吸附过程的常用方法之一。  
在给定温度 $T$、化学势 $\mu$（或压力 $P$）条件下，体系粒子数 $N$ 可以波动。  
